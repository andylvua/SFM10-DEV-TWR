...
typedef struct __attribute__ ((__packed__)) RecalcRxCmd {
  ...
  sensitivity. A higher value increases security while it also results in sensitivity loss.
   The attribute type is [uint8]. The value(s) should be between 0 and 176. */
  uint8_t level;
...
input first path security level.
 *
 * @param[in] level (uint8) The first path security level allows to make a trade-off between security and sensitivity. A higher value increases security while it also results in sensitivity loss. Its value should be between 0 and 176.
 * @param[out] uciStatus (enum uci status) A generic UCI status code. The value of 0x00 
indicates no error occurred.
 */
