...
**Prototype**: ``void sleep( uint32_t time, uint16_t pin, uint8_t pinEdge )``
...
### ``deepSleep(pin)``
**Prototype**: ``void deepSleep( uint16_t pin )``
...
**Prototype**: ``void limitCurrent( uint8_t milliAmps )``
...
Ram usage functions.
This family of functions has five functions defined.


### ``stackCapacity(stack)``
**Prototype**: ``uint16_t stackCapacity( uint8_t stack )``

**Description**: Returns number of bytes available for the main thread stack.


|Parameter|     |
|:--|:-----------------|
|stack | (uint8_t) Choice of stack. For supported values, [see below](#stack). | 
...
|Value  |(uint8_t)|
|:-|:-----------------|
|USER_STACK (0) | User thread, used for setup() and loop(). |
|IDLE_STACK (1) | Idle thread, used in attachInterrupt(IDLE, foo, 0). |
|INTERRUPT_STACK (2) | Interrupt thread, used in attachInterrupt(other than IDLE, foo, 0) and UWB.on functions. |


### ``stackUsageMax(stack)``
**Prototype**: ``uint16_t stackUsageMax( uint8_t stack )``

**Description**: Returns number of bytes currently used for the main thread stack.


|Parameter|     |
|:--|:-----------------|
|stack | (uint8_t) Choice of stack, see stackCapacity(). | 



### ``stackUsageCurrent(stack)``
**Prototype**: ``uint16_t stackUsageCurrent( uint8_t stack )``
...
### ``globalsCapacity()``
**Prototype**: ``uint16_t globalsCapacity( void )``

**Description**: Returns number of bytes available for global variables.



### ``globalsUsage()``
**Prototype**: ``uint16_t globalsUsage( void )``

**Description**: Returns number of bytes used for global variables.
...
**Prototype**: ``void pinMode( uint16_t pinMask, uint8_t mode )``
...
|:-|:---|:---|:---|
| ``pinMode(D8, OUTPUT)`` | 2 us | 2 us | 92 | 
| ``pinMode(D8, INPUT_PULLDOWN)`` | 4 us | 3 us | 168 | 
| ``pinMode(D8, INPUT_PULLUP)`` | 4 us | 3 us | 168 | 
| ``pinMode(D8, INPUT)`` | 4 us | 3 us | 168 | 
...
| | |
|:--|:----------------|
|0x030101 | E_GPIO_PIN_MODE__PIN_CLAIMED: The specified pin already serves some role. | 
|0x030102 | E_GPIO_PIN_MODE__PIN_UNKNOWN: The specified pin number does not map to a physical gpio pin. | 


### ``digitalWrite(pinMask, value)``
**Prototype**: ``void digitalWrite( uint16_t pinMask, uint8_t value )``
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``digitalWrite(D1, HIGH)`` | 2 us | 1 us | 76 | 
| ``digitalWrite(D1, LOW)`` | 2 us | 1 us | 71 | 
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``digitalRead(PIN_USER_BUTTON)`` | 1 us | 1 us | 57 | 
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``analogRead(VDD_IO)`` | 115 us | 80 us | 4418 | 
| ``analogRead(INT_TEMPERATURE)`` | 103 us | 72 us | 3949 | 
...
**Error Codes: **
In case of an error, ``delay()`` will set [``errno``](#errno) to one of the following error codes:

| | |
|:--|:----------------|
|0x040501 | E_TIMING_DELAY__FROM_ISR: Calling the delay function from inside an interrupt service routine is not possible (e.g., ``Timer.at()``). | 
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``delayMicroseconds(0)`` | 7 us | 5 us | 286 | 
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``getXtalPpb(FROM_TEMPERATURE)`` | 309 us | 215 us | 11849 | 
...
**Prototype**: ``void Timer.at( uint32_t timeMs, void (*isr)(void) )``
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``Timer.at(3000, loop)`` | 13 us | 9 us | 501 | 
...
**Prototype**: ``void Timer.atMicroseconds( uint64_t timeUs, void (*isr)(void) )``
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``Timer.atMicroseconds(3000000, loop)`` | 13 us | 9 us | 493 | 
...
**Prototype**: ``void Timer.atTicks( uint64_t timeTicks, void (*isr)(void) )``
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``Timer.atTicks(63897600000, loop)`` | 12 us | 9 us | 479 | 
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``Timer.every(3000, loop)`` | 9 us | 6 us | 346 | 
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``Timer.everyMicroseconds(3000000, loop)`` | 9 us | 6 us | 335 | 
...
**Prototype**: ``void attachInterrupt( uint16_t source, void (*isr)(void), int16_t mode )``
...
|mode | (int16_t) Defines when the interrupt should be triggered. For example on the RISING edge of a GPIO pin. For supported values, [see below](#mode). | 
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``attachInterrupt(PIN_USER_BUTTON, loop, FALLING)`` | 6 us | 4 us | 223 | 
| ``attachInterrupt(UWB_RX_SFD, loop, 0)`` | 2 us | 1 us | 81 | 
...
|IDLE (0xD000) | Triggers when the main thread is idle (i.e. during a waitForEvent()). |
...
**Prototype**: ``void detachInterrupt( uint16_t source )``
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``detachInterrupt(PIN_USER_BUTTON)`` | 3 us | 2 us | 106 | 
| ``detachInterrupt(UWB_RX_SFD)`` | 2 us | 2 us | 83 | 
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``interrupts(counter)`` | 2 us | 1 us | 64 | 
...
**Error Codes: **
In case of an error, ``triggerEvent()`` will set [``errno``](#errno) to one of the following error codes:

| | |
|:--|:----------------|
|0x060601 | E_INTERRUPT_TRIGGER_EVENT__NOT_WAITING: Trigger event was called before ``waitForEvent()`` was called. | 
...
This family of functions has eleven functions defined.
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``Serial.begin(115200)`` | 22 us | 15 us | 852 | 
...
| | |
|:--|:----------------|
|0x070101 | E_SERIAL_BEGIN__FAILED: Serial has already begun. In case the baud rate needs to be changed, call ``Serial.end()`` first. | 
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``Serial.write("2\n", 2)`` | 174 us | 121 us | 6669 | 
| ``Serial.write("1000\n", 5)`` | 435 us | 302 us | 16696 | 
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``Serial.setTimeout(2000)`` | 1 us | 1 us | 48 | 
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``Serial.printInt64(1, 0)`` | 86 us | 60 us | 3313 | 
| ``Serial.printInt64(1e3, 0)`` | 346 us | 241 us | 13302 | 
| ``Serial.printInt64(3e17, 0)`` | 1576 us | 1096 us | 60501 | 
...
**Prototype**: ``void SPI.transfer( uint8_t* txBuffer, uint8_t* rxBuffer, uint16_t length )``
...
### ``Crypto.begin()``
**Prototype**: ``void Crypto.begin( void )``
...
**Prototype**: ``void Crypto.randomBytes( uint8_t *const destination, uint32_t size )``
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``Crypto.randomBytes(buf1, 8)`` | 105 us | 73 us | 4050 | 
| ``Crypto.randomBytes(buf2, 64)`` | 229 us | 159 us | 8778 | 
| ``Crypto.randomBytes(buf3, 256)`` | 421 us | 293 us | 16160 | 
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``Crypto.configCrc(0x4C11DB7, 0xFFFFFFFF, true, 0xFFFFFFFF)`` | 2 us | 2 us | 88 | 
...
**Prototype**: ``void Crypto.configEncryption( uint8_t* keyValue, uint8_t keyLength, uint16_t algorithm, uint8_t* ivValue, uint8_t ivLength, uint8_t cipherMode, uint8_t* aadValue, uint8_t aadLength )``
...
**Prototype**: ``void Crypto.configDecryption( uint8_t* keyValue, uint8_t keyLength, uint16_t algorithm, uint8_t* ivValue, uint8_t ivLength, uint8_t cipherMode, uint8_t* aadValue, uint8_t aadLength )``
...
### ``Crypto.encrypt(...)``
**Prototype**: ``void Crypto.encrypt( uint8_t* source, uint16_t length, uint8_t* destination, uint32_t* authenticationTag )``
...
### ``Crypto.decrypt(...)``
**Prototype**: ``void Crypto.decrypt( uint8_t* source, uint16_t length, uint8_t* destination, uint32_t* authenticationTag )``
...
**Prototype**: ``void EEPROM.write( uint16_t address, uint8_t value )``
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``EEPROM.write(0, 0x33)`` | 3272 us | 2276 us | 125652 | 
...
**Prototype**: ``void EEPROM.readBytes( uint8_t* buffer, uint16_t address, uint16_t length )``
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``EEPROM.readBytes(buf, 0, 8)`` | 14 us | 10 us | 542 | 
| ``EEPROM.readBytes(buf, 0, 256)`` | 28 us | 20 us | 1078 | 
...
**Prototype**: ``void EEPROM.writeBytes( uint8_t* buffer, uint16_t address, uint16_t length )``
...
|Call  | Duration | Session active | Cycles |
|:-|:---|:---|:---|
| ``EEPROM.writeBytes(buf, 0, 8)`` | 3272 us | 2276 us | 125639 | 
| ``EEPROM.writeBytes(buf, 0, 256)`` | 3342 us | 2325 us | 128332 | 
...
|Input|     |
|:--|:-----------------|
|level | (0<=uint8<=176) The first path security level allows to make a trade-off between security and sensitivity. A higher value increases security while it also results in sensitivity loss.|
