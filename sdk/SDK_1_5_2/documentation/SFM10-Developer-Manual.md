...
### ``StackUsage.begin()``
**Prototype**: ``void StackUsage.begin( void )``

**Description**: Fills the stack area with a dedicated pattern in order to find the usage later.



### ``StackUsage.findMax()``
**Prototype**: ``uint16_t StackUsage.findMax( void )``

**Description**: Returns how far the stack has been used since begin() was called.



### ``StackUsage.current()``
**Prototype**: ``uint16_t StackUsage.current( void )``
...
### ``StackUsage.limit()``
**Prototype**: ``uint16_t StackUsage.limit( void )``

**Description**: Returns total stack area (constant).
...
| | |
|:--|:----------------|
|0x030101 | E_GPIO_PIN_MODE__PIN_UNKNOWN: The specified pin number does not map to a physical gpio pin. | 
...
| | |
|:--|:----------------|
|0x030201 | E_GPIO_DIGITAL_WRITE__PIN_UNKNOWN: The specified pin number does not map to a physical GPIO pin. | 
|0x030202 | E_GPIO_DIGITAL_WRITE__PIN_IS_INPUT: The specified pin is configured as an output. Use ``pinMode()`` to make it an INPUT and optionally activate pullup or pulldown resistors. | 
...
| | |
|:--|:----------------|
|0x030301 | E_GPIO_DIGITAL_READ__PIN_UNKNOWN: The specified pin number does not map to a physical GPIO pin. | 
|0x030302 | E_GPIO_DIGITAL_READ__PIN_IS_OUTPUT: The specified pin number is configured as an output. | 
...
| | |
|:--|:----------------|
|0x030401 | E_GPIO_ANALOG_READ__NOT_READY: The ADC is not yet ready. | 


## Timing functions


This family of functions has six functions defined.
...
| | |
|:--|:----------------|
|0x050401 | E_TIMER_AT__QUEUE_FULL: Cannot schedule another event because the queue is maxed out. | 
...
| | |
|:--|:----------------|
|0x050501 | E_TIMER_AT_MICROSECONDS__OVERFLOW: Cannot schedule because the period between now (see micros()) and the provided time is too long. | 
...
| | |
|:--|:----------------|
|0x050601 | E_TIMER_AT_TICKS__OVERFLOW: Cannot schedule because the period between now (see micros()) and the provided time is too long. | 
...
This family of functions has ten functions defined.
...
|UWB_TX_DONE_MARKER (0x7000) | Triggers when TX has completed. |
|UWB_RX_SFD_MARKER (0x8000) | Triggers when the Start of Frame Delimiter (SFD) is received. THE SFD is positioned between the SYNC (preamble) part and the PHR part. The SFD part is used for time-stamping which is useful for ranging. |
|UWB_RX_PHR_MARKER (0x9000) | Triggers when the PHR is received. PHR stands for Physical Layer Header. It provides information to the receiver about the transmitted payload and other useful physical layer information. |
|UWB_RX_START_MARKER (0xA000) | Triggers when RX has started. |
|UWB_RX_EOF_MARKER (0xB000) | Triggers when the End of Frame (EOF) is detected. |
|UWB_VBUF_LOW_INTERRUPT (0xC000) | Triggers when the the voltage of VBUF is too low. Adjust current limiter or add capacitance to resolve this. |
|SERIAL_WRITE_COMPLETE (0xD000) | The ISR will be executed every time a asynchronous TX was successful. |
|UWB_TX_INTERRUPT (0xC100) | The ISR will be executed every time TX result(s) are ready to be read out. Equivalent to UWB.onTX(myEvent, NULL). |
|UWB_RX_INTERRUPT (0xC200) | The ISR will be executed every time RX result(s) are ready to be read out. Equivalent to UWB.onRX(myEvent, NULL, NULL...). |
|UWB_LOOPBACK_INTERRUPT (0xC300) | The ISR will be executed every time loopback result(s) are ready to be read out. |
...
| | |
|:--|:----------------|
|0x060101 | E_INTERRUPT_ATTACH_INTERRUPT__WRONG_NUM: The specified type of interrupt is not valid. | 
|0x060102 | E_INTERRUPT_ATTACH_INTERRUPT__GPIO_OUTPUT: The specified GPIO pin is configured as an output. Use ``pinMode()`` to make it an input first. | 
|0x060103 | E_INTERRUPT_ATTACH_INTERRUPT__TOO_MANY: Too many interrupts are scheduled. Consider removing unused ones via ``detachInterrupt()``. | 
|0x060104 | E_INTERRUPT_ATTACH_INTERRUPT__WRONG_MODE: The specified mode is not supported. | 
|0x060105 | E_INTERRUPT_ATTACH_INTERRUPT__PIN_CLAIMED: The specified GPIO pin has an ISR attached. | 
...
| | |
|:--|:----------------|
|0x060201 | E_INTERRUPT_DETACH_INTERRUPT__WRONG_NUM: The specified type of interrupt is not valid. | 
...
| | |
|:--|:----------------|
|0x060301 | E_INTERRUPT_NO_INTERRUPTS__WRONG_ORDER: The counter value passed to noInterrupts was not the latest one. | 
...
### ``triggerEventBit(eventBitIndex)``
**Prototype**: ``void triggerEventBit( uint8_t eventBitIndex )``

**Description**: Like, triggerEvent() but faster. .


|Parameter|     |
|:--|:-----------------|
|eventBitIndex | (uint8_t) The bit corresponding with the provided zero-based index will be returned by the pending ``waitForEvent()`` function. The value(s) should be between 0 and 31. | 



### ``countEvent(eventBitIndex)``
**Prototype**: ``uint32_t countEvent( uint8_t eventBitIndex )``

**Description**: Returns the number of times the specified bit index was triggered.


|Parameter|     |
|:--|:-----------------|
|eventBitIndex | (uint8_t) To select the event mask bit of interrest. The index is zero-based, e.g. index=1 is to count triggerEvent(0x2) occurrences. The value(s) should be between 0 and 31. | 
...
This family of functions has thirteen functions defined.
...
**Prototype**: ``ErrnoVal Serial.write( const uint8_t* buffer, uint16_t length )``
...
| | |
|:--|:----------------|
|0x070501 | E_SERIAL_WRITE__BUSY: Cannot perform write at this moment. Still busy writing previous. | 


### ``Serial.busyWriting()``
**Prototype**: ``bool Serial.busyWriting( void )``

**Description**: Returns whether UART is currently busy with transmission. Catch SERIAL_WRITE_COMPLETE interrupt.
...
| | |
|:--|:----------------|
|0x080501 | E_SPI_TRANSFER__BUSY: Indicates that the a SPI transaction is still being performed. | 
...
| | |
|:--|:----------------|
|0x0B0101 | E_CRYPTO_BEGIN__FAILED: Couldn't power up the crypto subsystem. Check the power connections and power source. | 
...
| | |
|:--|:----------------|
|0x0B0201 | E_CRYPTO_RANDOM_BYTES__BUSY: Indicates that the security subsystem was currently busy performing cryptographic operations and, therefore, could not process the operation. | 
|0x0B0202 | E_CRYPTO_RANDOM_BYTES__GENERAL_FAILURE: Indicates that the random number generator failed due to an unspecified internal error condition. | 
|0x0B0203 | E_CRYPTO_RANDOM_BYTES__NOT_BEGAN: The cryptographic subsystem has not powered up. Call ``Crypto.begin()`` first. | 
...
| | |
|:--|:----------------|
|0x0B0401 | E_CRYPTO_CALC_CRC__READ: Indicates that the CRC calculation was terminated prematurely due to read error on the input data. This typically indicates that an attempt was made to calculate a CRC over a flash memory region containing erased or corrupted flash pages. | 
|0x0B0402 | E_CRYPTO_CALC_CRC__BAD_PARAM: Indicates that an invalid parameter was passed. | 
|0x0B0403 | E_CRYPTO_CALC_CRC__UNDEFINED: Invalid data. | 
...
| | |
|:--|:----------------|
|0x0B0501 | E_CRYPTO_CONFIG_ENCRYPTION__BUSY: Indicates that the security subsystem was currently busy performing cryptographic operations and, therefore, could not process the import key operation. | 
|0x0B0502 | E_CRYPTO_CONFIG_ENCRYPTION__BAD_PARAMETER: Indicates that at least one of the parameters of this function were invalid. | 
|0x0B0503 | E_CRYPTO_CONFIG_ENCRYPTION__NOT_PERMITTED: Indicates that the raw key import operation is not allowed for the requested key and/or the current platform state. | 
|0x0B0504 | E_CRYPTO_CONFIG_ENCRYPTION__GENERAL_ERROR: Indicates that the import key operation failed due to an unspecified internal error condition. No further error information is provided, in order to minimize the amount of information that is exposed to potential attackers. | 
...
| | |
|:--|:----------------|
|0x0B0601 | E_CRYPTO_CONFIG_DECRYPTION__BUSY: Indicates that the security subsystem was currently busy performing cryptographic operations and, therefore, could not process the import key operation. | 
|0x0B0602 | E_CRYPTO_CONFIG_DECRYPTION__BAD_PARAMETER: Indicates that at least one of the parameters of this function were invalid. | 
|0x0B0603 | E_CRYPTO_CONFIG_DECRYPTION__NOT_PERMITTED: Indicates that the raw key import operation is not allowed for the requested key and/or the current platform state. | 
|0x0B0604 | E_CRYPTO_CONFIG_DECRYPTION__GENERAL_ERROR: Indicates that the import key operation failed due to an unspecified internal error condition. No further error information is provided, in order to minimize the amount of information that is exposed to potential attackers. | 
...
| | |
|:--|:----------------|
|0x0B0701 | E_CRYPTO_ENCRYPT__BUSY: Indicates that the security subsystem was currently busy performing cryptographic operations and, therefore, could not process the import key operation. | 
|0x0B0702 | E_CRYPTO_ENCRYPT__BAD_PARAMETER: Indicates that at least one of the parameters of this function were invalid. | 
|0x0B0703 | E_CRYPTO_ENCRYPT__NOT_PERMITTED: Indicates that the raw key import operation is not allowed for the requested key and/or the current platform state. | 
|0x0B0704 | E_CRYPTO_ENCRYPT__GENERAL_ERROR: Indicates that the import key operation failed due to an unspecified internal error condition. No further error information is provided, in order to minimize the amount of information that is exposed to potential attackers. | 
...
| | |
|:--|:----------------|
|0x0B0801 | E_CRYPTO_DECRYPT__VERIFICATION_FAILED: Is reported by authenticated cipher suites, such AES in CCM mode, if verification of the authentication tag failed. This error indicates that the integrity of the provided ciphertext (and/or its authentication tag) has been compromised and, therefore, that the ciphertext is not trustworthy. | 
|0x0B0802 | E_CRYPTO_DECRYPT__BUSY: Indicates that the security subsystem was currently busy performing cryptographic operations and, therefore, could not process the import key operation. | 
|0x0B0803 | E_CRYPTO_DECRYPT__BAD_PARAMETER: Indicates that at least one of the parameters of this function were invalid. | 
|0x0B0804 | E_CRYPTO_DECRYPT__NOT_PERMITTED: Indicates that the raw key import operation is not allowed for the requested key and/or the current platform state. | 
|0x0B0805 | E_CRYPTO_DECRYPT__GENERAL_ERROR: Indicates that the import key operation failed due to an unspecified internal error condition. No further error information is provided, in order to minimize the amount of information that is exposed to potential attackers. | 
...
| | |
|:--|:----------------|
|0x0C0101 | E_EEPROM_READ__OUT_OF_RANGE: The target address is out of range. | 
...
| | |
|:--|:----------------|
|0x0C0201 | E_EEPROM_WRITE__FAILED: General flash write error. | 
|0x0C0202 | E_EEPROM_WRITE__OUT_OF_RANGE: The target address is out of range. | 
...
| | |
|:--|:----------------|
|0x0C0301 | E_EEPROM_READ_BYTES__FAILED: Reading failed, possibly because no data is stored at the target address. | 
|0x0C0302 | E_EEPROM_READ_BYTES__OUT_OF_RANGE: The target address is out of range. | 
...
| | |
|:--|:----------------|
|0x0C0401 | E_EEPROM_WRITE_BYTES__FAILED: General flash write error. | 
|0x0C0402 | E_EEPROM_WRITE_BYTES__OUT_OF_RANGE: The target address is out of range. | 


## Aliases

UWB alias functions are added to simplify the usage of UWB class functions.
This family of functions has two functions defined.
...
**Prototype**: ``UciStatus setPayload( uint8_t* buffer, uint16_t length )``

**Description**: Define payload data and payload length as separate function arguments. See also UWB.setPayload().
...
**Description**: Returns not 0 when UWB action is being executed.
...
### ``UWB.readTxTime(output)``
**Prototype**: ``
UciStatus readTxTime(ReadTxTimeRsp* output)``

**Description**: To get the most recently captured (RX/TX) timestamp event.
...
|:--|:-----------------|
|uciStatus | (enum) A generic UCI status code. The value of 0x00 indicates no error occurred. Please see the [Uci status table](#uci-status) below.|
|txTimestamp | (uint64) The captured Timestamp timer value for when the message is transmitted (TX). The units are in ticks and to convert its value to seconds you have to multiply it by 1/(128*499.2e6). Units are [ticks].|

### ``UWB.readRxTime(output)``
**Prototype**: ``
UciStatus readRxTime(ReadRxTimeRsp* output)``
...
|:--|:-----------------|
|uciStatus | (enum) A generic UCI status code. The value of 0x00 indicates no error occurred. Please see the [Uci status table](#uci-status) below.|
|rxTimestamp | (uint64) The captured Timestamp timer value for when the message is received (RX). The units are in ticks and to convert its value to seconds you have to multiply it by 1/(128*499.2e6). Units are [ticks].|
|rxCarrierFreqOffset | (int16) The device in Receive operation computes the frequency offset(CFO_INT) between transmitter and receiver carrier freq. This value will be provided after the Rx operation. To compute the ppm from this CFO_INT value, the following are the steps: A) Compute CFO in rad/pi per clock cycle: CFO = CFO_INT/2^17. B) Compute CFO(Hz) = (124.8MHz/2)*CFO. C) Compute CFO(ppm) = CFO(Hz) / (fc/1e6), where fc is the carrier frequency and for channel 9 it will be 7.9872GHz.|

### ``UWB.readLoopbackTime(output)``
**Prototype**: ``
UciStatus readLoopbackTime(ReadLoopbackTimeRsp* output)``
...
### ``UWB.readPayload(output)``
**Prototype**: ``
UciStatus readPayload(ReadPayloadRsp* output)``

**Description**: Returns the most recently received (RX) payload.
...
### ``UWB.readFirstPathInfo(output)``
**Prototype**: ``
UciStatus readFirstPathInfo(ReadFirstPathInfoRsp* output)``

**Description**: To get the most recently captured (RX) signal strength.
...
|:--|:-----------------|
|uciStatus | (enum) A generic UCI status code. The value of 0x00 indicates no error occurred. Please see the [Uci status table](#uci-status) below.|
|firstPathDetected | (uint32) Indicates the first path was detected and the ToA estimation results are valid. The first path detected status is indicated at the least significant bit position. i.e b_firstPathdetected = (bool) (firstPathdetected & 0x1).|
|rfu1 | (uint32) Reserved for future use.|
|edgeIdx | (uint32) Index of first crossing of the detection threshold.|
|fpTime | (int32) Interpolated ToA result (first path lead distance) in samples. ToA result = time / 2^15. The first path lead distance is the distance between the end of CIR and the estimated first path.|
|maxTapIndex | (uint16) Index of strongest path. Variable maxTapIndex is computed from simplified approximation in HW instead of complex CIR computation.|
|maxTapPower | (uint16) Power at the sample specified by maxTapIndex. Level[dBm] = (log10(2) * 10 * (float)(int16_t)maxTapPower) / 0x200.|
|firstPathIndex | (uint16) Index of first path.|
|firstPathPower | (int16) Power of the first path. The first-path power is measured at the first-path index found by the ToA algorithm. Level[dBm] = (log10(2) * 10 * (float)(int16_t)firstPathPower) / 0x200.|
|noisePower | (uint32) Noise power estimate. Level[dBm] = (log10(2) * 10 * (float)(int16_t)noisePower) / 0x2000000.|
|detectThrPower | (int32) Detection threshold power level. Level[dBm] = (log10(2) * 10 * (float)(int16_t)detectThrPower) / 0x2000000.|
|rfu2 | (uint32) Reserved Fields.|
|overallRxPower | (int32) Overall received power computed by the Rx front end. Level[dBm] = (log10(2) * 10 * (float)overallRxPower) / 0x2000000.|
|phr | (uint32) Structure for the Physical Header (PHR) field of the UWB frame.|

### ``UWB.readCir(input, output)``
**Prototype**: ``
UciStatus readCir(ReadCirCmd* input, ReadCirRsp* output)``
