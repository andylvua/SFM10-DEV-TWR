...
#include <stdint.h>
#include "stdbool.h"
#define STANDALONE_API_FUNCTION_COUNT    82
#define STANDALONE_API_FULL_DATE 0x24091882

#define STANDALONE_API_VERSION_MAJOR 1
#define STANDALONE_API_VERSION_MINOR 4
#define STANDALONE_API_VERSION_PATCH 8
#define STANDALONE_API_VERSION_DATE ((24-20) * 0x10 + 9)
...
now (see micros()) and the provided time is too long. */
  E_TIMER_AT_MICROSECONDS__OVERFLOW                = 0x040501,
...
as an output. Use ``pinMode()`` to make it an input first. */
  E_INTERRUPT_ATTACH_INTERRUPT__GPIO_OUTPUT        = 0x050102,
...
operation. */
  E_CRYPTO_RANDOM_BYTES__BUSY                      = 0x0A0201,
...
import key operation. */
  E_CRYPTO_CONFIG_ENCRYPTION__BUSY                 = 0x0A0501,
...
parameters of this function were invalid. */
  E_CRYPTO_CONFIG_ENCRYPTION__BAD_PARAMETER        = 0x0A0502,
...
operation is not allowed for the requested key and/or the current platform state. */
  E_CRYPTO_CONFIG_ENCRYPTION__NOT_PERMITTED        = 0x0A0503,
...
not allowed for the requested key and/or the current platform state. */
  E_CRYPTO_ENCRYPT__NOT_PERMITTED                  = 0x0A0703,
...
order to minimize the amount of information that is exposed to potential attackers. */
  E_CRYPTO_ENCRYPT__GENERAL_ERROR                  = 0x0A0704,
