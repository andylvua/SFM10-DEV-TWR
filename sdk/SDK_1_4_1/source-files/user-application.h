...
#include <stdint.h>
#include "stdbool.h"
#define STANDALONE_API_FUNCTION_COUNT    86
#define STANDALONE_API_FULL_DATE 0x24070586

#define STANDALONE_API_VERSION_MAJOR 1
#define STANDALONE_API_VERSION_MINOR 4
#define STANDALONE_API_VERSION_PATCH 1
#define STANDALONE_API_VERSION_DATE ((24-20) * 0x10 + 7)
...
#define ERROR_EVENT ERROR_INTERRUPT
#define GPIO_EVENT GPIO_INTERRUPT
#define SERIAL_EVENT SERIAL_INTERRUPT
// The three definitions above have been renamed to more clearly distinguish
// between interrupts (defined in the library) and events (defined by the user).
#define DPD_GPIO_EVENT DPD_GPIO
 ...
 */
extern const void (*sleep)(uint32_t time, uint16_t pin, uint8_t pinEdge);
 ...
 * @param pin (uint16_t) Wake-up pin configuration. Please note that at least one wake up source has to be configured. If no wake up source is configured a parameter error will be returned.
 */
extern const void (*deepSleep)(uint16_t pin);
 ...
 */
extern const void (*limitCurrent)(uint8_t milliAmps);
...
/** @brief User thread, used for setup() and loop(). */
#define USER_STACK                                         0
 
/** @brief Idle thread, used in attachInterrupt(IDLE, foo, 0). */
#define IDLE_STACK                                         1
 
/** @brief Interrupt thread, used in attachInterrupt(other than IDLE, foo, 0) and UWB.on 
functions. */
#define INTERRUPT_STACK                                    2

/**
 * @brief Returns number of bytes available for the main thread stack.
 *
 * @param stack (uint8_t) Choice of stack.
 *
 * @return byteCount (uint16_t) .
 */
extern const uint16_t (*stackCapacity)(uint8_t stack);

/**
 * @brief Returns number of bytes currently used for the main thread stack.
 *
 * @param stack (uint8_t) Choice of stack, see stackCapacity().
 *
 * @return byteCount (uint16_t) .
 */
extern const uint16_t (*stackUsageMax)(uint8_t stack);
 ...
 * @return byteCount (uint16_t) .
 */
extern const uint16_t (*stackUsageCurrent)(uint8_t stack);

/**
 * @brief Returns number of bytes available for global variables.
 *
 *
 * @return byteCount (uint16_t) .
 */
extern const uint16_t (*globalsCapacity)(void);

/**
 * @brief Returns number of bytes used for global variables.
 *
 *
 * @return byteCount (uint16_t) .
 */
extern const uint16_t (*globalsUsage)(void);
 ...
 */
extern const void (*pinMode)(uint16_t pinMask, uint8_t mode);
 ...
 */
extern const void (*digitalWrite)(uint16_t pinMask, uint8_t value);
 ...
 */
  void (*at)(uint32_t timeMs, void (*isr)(void));
 ...
 * @param function (void (*isr)(void)) The function that is called at the specified time. This function must take no parameters and return nothing.
 */
  void (*atMicroseconds)(uint64_t timeUs, void (*isr)(void));
 ...
 * @param function (void (*isr)(void)) The function that is called at the specified time. This function must take no parameters and return nothing.
 */
  void (*atTicks)(uint64_t timeTicks, void (*isr)(void));
 ...
 * @param mode (int16_t) Defines when the interrupt should be triggered. For example on the RISING edge of a GPIO pin.
 */
extern const void (*attachInterrupt)(uint16_t source, void (*isr)(void), int16_t mode);
 ...
 */
extern const void (*detachInterrupt)(uint16_t source);
 ...
 *
 * @return byteCount (int) Total number of bytes written.
 */
 ...
 */
  void (*transfer)(uint8_t* txBuffer, uint8_t* rxBuffer, uint16_t length);
 ...
 */
  void (*randomBytes)(uint8_t *const destination, uint32_t size);
 ...
 */
  void (*configEncryption)(uint8_t* keyValue, uint8_t keyLength, uint16_t algorithm, uint8_t* ivValue, uint8_t ivLength, uint8_t cipherMode, uint8_t* aadValue, uint8_t aadLength);
 ...
 * @param aadLength (uint8_t) Length of the provided AAD value (number of bytes).
 */
  void (*configDecryption)(uint8_t* keyValue, uint8_t keyLength, uint16_t algorithm, uint8_t* ivValue, uint8_t ivLength, uint8_t cipherMode, uint8_t* aadValue, uint8_t aadLength);
 ...
 */
  void (*encrypt)(uint8_t* source, uint16_t length, uint8_t* destination, uint32_t* authenticationTag);
 ...
 */
  void (*decrypt)(uint8_t* source, uint16_t length, uint8_t* destination, uint32_t* authenticationTag);
 ...
 */
  void (*readBytes)(uint8_t* buffer, uint16_t address, uint16_t length);
 ...
 * @param length (uint16_t) The number of bytes to read. Its value should be between 1 and 1023.
 */
  void (*writeBytes)(uint8_t* buffer, uint16_t address, uint16_t length);
...
#define E_GPIO_DIGITAL_WRITE__PIN_IS_INPUT            0x030202

/** @brief Could not execute GPIO.digitalRead(). The specified pin number does not map to a 
physical GPIO pin. */
#define E_GPIO_DIGITAL_READ__PIN_UNKNOWN              0x030301

/** @brief Could not execute GPIO.digitalRead(). The specified pin number is configured as an 
output. */
#define E_GPIO_DIGITAL_READ__PIN_IS_OUTPUT            0x030302

/** @brief Could not execute GPIO.analogRead(). The ADC is not yet ready. */
#define E_GPIO_ANALOG_READ__NOT_READY                 0x030401

/** @brief Could not execute Timing.delay(). Calling the delay function from inside an 
interrupt service routine is not possible (e.g., ``Timer.at()``). */
#define E_TIMING_DELAY__FROM_ISR                      0x040501

/** @brief Could not execute Timer.at(). Cannot schedule another event because the queue is 
maxed out. */
#define E_TIMER_AT__QUEUE_FULL                        0x050401
...
#define E_TIMER_AT_MICROSECONDS__OVERFLOW             0x050501
...
#define E_TIMER_AT_TICKS__OVERFLOW                    0x050601

/** @brief Could not execute Interrupt.attachInterrupt(). The specified type of interrupt is 
not valid. */
#define E_INTERRUPT_ATTACH_INTERRUPT__WRONG_NUM       0x060101
...
#define E_INTERRUPT_ATTACH_INTERRUPT__GPIO_OUTPUT     0x060102
...
#define E_INTERRUPT_ATTACH_INTERRUPT__TOO_MANY        0x060103

/** @brief Could not execute Interrupt.attachInterrupt(). The specified mode is not supported. */
#define E_INTERRUPT_ATTACH_INTERRUPT__WRONG_MODE      0x060104

/** @brief Could not execute Interrupt.attachInterrupt(). The specified GPIO pin has an ISR 
attached. */
#define E_INTERRUPT_ATTACH_INTERRUPT__PIN_CLAIMED     0x060105

/** @brief Could not execute Interrupt.detachInterrupt(). The specified type of interrupt is 
not valid. */
#define E_INTERRUPT_DETACH_INTERRUPT__WRONG_NUM       0x060201

/** @brief Could not execute Interrupt.noInterrupts(). The counter value passed to noInterrupts 
was not the latest one. */
#define E_INTERRUPT_NO_INTERRUPTS__WRONG_ORDER        0x060301

/** @brief Could not execute Interrupt.triggerEvent(). Trigger event was called before 
``waitForEvent()`` was called. */
#define E_INTERRUPT_TRIGGER_EVENT__NOT_WAITING        0x060601

/** @brief Could not execute Serial.begin(). Serial has already begun. In case the baud rate 
needs to be changed, call ``Serial.end()`` first. */
#define E_SERIAL_BEGIN__FAILED                        0x070101

/** @brief Could not execute SPI.transfer(). Indicates that the a SPI transaction is still 
being performed. */
#define E_SPI_TRANSFER__BUSY                          0x080501

/** @brief Could not execute Crypto.begin(). Couldn't power up the crypto subsystem. Check the 
power connections and power source. */
#define E_CRYPTO_BEGIN__FAILED                        0x0B0101
...
operation. */
#define E_CRYPTO_RANDOM_BYTES__BUSY                   0x0B0201
...
#define E_CRYPTO_CONFIG_ENCRYPTION__BUSY              0x0B0501
...
#define E_CRYPTO_CONFIG_ENCRYPTION__BAD_PARAMETER     0x0B0502
...
#define E_CRYPTO_CONFIG_ENCRYPTION__NOT_PERMITTED     0x0B0503
...
#define E_CRYPTO_ENCRYPT__NOT_PERMITTED               0x0B0703
...
#define E_CRYPTO_ENCRYPT__GENERAL_ERROR               0x0B0704
